    ...
    <div class="flex items-center gap-3">
        <a href="/games"
           class="flex items-center gap-2 border border-white/20 px-5 py-2 text-[11px] tracking-[0.15em] font-valo font-semibold uppercase hover:border-white/50 hover:text-white transition-all duration-200 group">
            <i data-lucide="layout-grid" class="w-4 h-4 text-white/60 group-hover:text-white transition-colors"></i>
            Missions
        </a>
        <a href="/profile/achievements"
           class="flex items-center gap-2 border border-white/20 px-5 py-2 text-[11px] tracking-[0.15em] font-valo font-semibold uppercase hover:border-white/50 hover:text-white transition-all duration-200 group">
            <i data-lucide="trophy" class="w-4 h-4 text-white/60 group-hover:text-white transition-colors"></i>
            Succès
        </a>
        <?php if (!empty($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin'): ?>
            <a href="/admin/users"
               class="flex items-center gap-2 border border-white/20 px-5 py-2 text-[11px] tracking-[0.15em] font-valo font-semibold uppercase hover:border-white/50 hover:text-white transition-all duration-200 group">
                <i data-lucide="shield" class="w-4 h-4 text-white/60 group-hover:text-white transition-colors"></i>
                Admin
            </a>
        <?php endif; ?>
        <a href="/logout"
...
<main class="flex-1 px-16 py-14">
    ...
    <div class="mb-14">
        ...
        <div class="flex items-end gap-8">
            <div class="w-20 h-20 rounded-2xl border border-white/10 bg-white/5 flex items-center justify-center shrink-0">
                <i data-lucide="user-round" class="w-10 h-10 text-white/30"></i>
            </div>
            <div>
                ...
                <div class="flex items-center gap-6 text-[13px]">
                    <span class="flex items-center gap-2 text-white/50">
                        <i data-lucide="mail" class="w-3.5 h-3.5 text-white/30"></i>
                        <?= htmlspecialchars($user->email) ?>
                    </span>
                    <span class="flex items-center gap-1.5 text-valo-red/80 text-[11px] font-valo tracking-[0.15em] uppercase border border-valo-red/30 px-2 py-0.5">
    ...
    <div class="flex items-center justify-between mb-1">
        ...
        <a href="/profile/games/add"
           class="flex items-center gap-2 border border-valo-red/40 px-4 py-1.5 text-[11px] tracking-[0.15em] font-valo font-semibold uppercase hover:border-valo-red/80 hover:text-valo-red transition-all duration-200 group">
            <i data-lucide="plus" class="w-3.5 h-3.5 text-valo-red/60 group-hover:text-valo-red transition-colors"></i>
            Ajouter un jeu
        </a>
    </div>
    ...
    <?php if (empty($gamesData)): ?>
        <div class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-white/30 text-sm font-valo tracking-[0.15em] uppercase">Aucune mission enregistrée</p>
        ...
        <div class="border border-white/5 rounded-2xl overflow-hidden">
            <table class="w-full text-sm">
                ...
                <tr class="border-b border-white/5 bg-white/2">
                    <th class="text-left px-5 py-3 text-[10px] font-valo tracking-[0.2em] text-white/30 uppercase font-semibold">Jeu</th>
                    <th class="text-left px-5 py-3 text-[10px] font-valo tracking-[0.2em] text-white/30 uppercase font-semibold">Type</th>
                    <th class="text-left px-5 py-3 text-[10px] font-valo tracking-[0.2em] text-white/30 uppercase font-semibold">Ajouté le</th>
                    <th class="text-left px-5 py-3 text-[10px] font-valo tracking-[0.2em] text-white/30 uppercase font-semibold">
                            <span class="flex items-center gap-1.5">
                                <i data-lucide="clock" class="w-3 h-3"></i>
                                Temps de jeu
                            </span>
                    </th>
                    <th class="text-left px-5 py-3 text-[10px] font-valo tracking-[0.2em] text-white/30 uppercase font-semibold">Actions</th>
                </tr>
                </thead>
                <tbody>
                            ...
                            <?php if ($game): ?>
                                <a href="/games/<?= $game->id ?>"
                                   class="font-valo text-sm tracking-wide hover:text-valo-red transition-colors duration-150 flex items-center gap-2">
                                    <i data-lucide="gamepad-2" class="w-4 h-4 text-white/20 group-hover:text-valo-red/40 transition-colors shrink-0"></i>
                                    <?= htmlspecialchars($game->name) ?>
                                </a>
                            <?php else: ?>
                                <span class="font-valo text-sm tracking-wide text-white/60 flex items-center gap-2">
