...
<main class="flex-1 flex flex-col md:flex-row">
    ...
    <div class="relative md:w-[42%] overflow-hidden flex flex-col justify-between p-8 md:p-12 bg-black border-b md:border-b-0 md:border-r border-white/8 min-h-55 md:min-h-0">
        ...
        <div class="absolute top-0 right-0 w-64 h-full"
             style="background: linear-gradient(to left, rgba(189,57,68,.05), transparent); clip-path: polygon(40% 0, 100% 0, 100% 100%, 0 100%);"></div>

        <!-- Coins décoratifs -->
        <div class="absolute top-6 right-6 w-8 h-8 border-t border-r border-valo-red/20"></div>
        ...
        <div class="relative z-10">
            <a href="/games/<?= $game->id ?>"
               class="inline-flex items-center gap-2 leading-none text-[11px] font-valo tracking-[0.15em] uppercase text-white/30 hover:text-white transition-colors duration-150">
                ...
                Retour à <?= htmlspecialchars($game->name) ?>
            </a>
        </div>

        <!-- Titre -->
        <div class="relative z-10 py-6 md:py-0">
            <p class="text-valo-red text-[10px] font-valo font-semibold tracking-[0.3em] uppercase mb-2">
                // <?= htmlspecialchars(strtoupper($game->name)) ?>
            </p>
            <p class="text-white/30 text-[10px] font-valo tracking-[0.25em] uppercase mb-4">PANNEAU ADMIN</p>
            <h1 class="font-valo font-bold text-4xl md:text-5xl tracking-[0.06em] leading-none mb-6">
                NOUVEAU<br><span class="text-valo-red">NIVEAU</span>
            </h1>
            <div class="w-8 h-px bg-valo-red/40 mb-5"></div>
            <p class="text-white/35 text-xs leading-relaxed max-w-xs">
                Ajoutez un niveau à ce protocole. Les joueurs pourront le consulter depuis la fiche du jeu.
            </p>
        </div>

        <!-- Status -->
        <div class="relative z-10 flex items-center gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-valo-red animate-pulse"></span>
            <span class="text-valo-red/50 text-[10px] font-valo tracking-[0.15em] uppercase">Accès Administrateur</span>
        </div>
    </div>
    ...
    <div class="flex-1 flex items-center justify-center p-8 md:p-12 lg:p-16">
        <div class="w-full max-w-lg">
            ...
            <form method="POST" action="/games/<?= $game->id ?>/levels" class="space-y-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="name" name="name" required placeholder="ex: Icebox"
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea id="description" name="description" rows="4"
                              placeholder="Brève description du niveau…"
                ...
                <div class="pt-2">
                    <button type="submit"
                            class="w-full bg-valo-red hover:bg-valo-red/80 active:scale-[.99] transition-all duration-150 py-3 text-sm font-valo font-bold tracking-[0.2em] uppercase text-white shadow-lg shadow-valo-red/15">
                        Créer le niveau
                    </button>
                </div>
